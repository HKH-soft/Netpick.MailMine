...
public class TransactionController {
    ...
    public ResponseEntity<Transaction> getTransaction(@PathVariable UUID id, @AuthenticationPrincipal User user) {
        ...
        // Verify ownership
        if (!transaction.getCreatedBy().equals(user.getId()) && !user.hasRole(RoleEnum.SUPER_ADMIN)) {
    ...
    public ResponseEntity<Transaction> updateTransaction(
        ...
        // Verify ownership
        if (!existing.getCreatedBy().equals(user.getId()) && !user.hasRole(RoleEnum.SUPER_ADMIN)) {
            throw new AccessDeniedException("Access denied to this transaction");
        }
