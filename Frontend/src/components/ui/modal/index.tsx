...
export const Modal: React.FC<ModalProps> = ({
  ...
  const contentClasses = isFullscreen
    ? "w-full h-full"
    : "relative w-full rounded-3xl bg-white dark:bg-gray-950";

  return (
    <div className="fixed inset-0 flex items-center justify-center overflow-y-auto modal z-99999" role="dialog" aria-modal="true">
      {!isFullscreen && (
        <div
          className="fixed inset-0 z-10 h-full w-full bg-black/40 backdrop-blur-sm dark:bg-black/40"
          onClick={onCloseAction}
        ></div>
      )}
      <div
        ref={modalRef}
        className={`${contentClasses} ${className} z-20`}
        style={{
